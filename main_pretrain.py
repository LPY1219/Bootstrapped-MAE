...
def get_args_parser():
    ...
    #总结来讲，梯度累加就是每计算一个batch的梯度，不进行清零，而是做梯度的累加(平均)，当累加到一定的次数之后，再更新网络参数，然后将梯度清零。通过这种参数延迟更新的手段，可以实现与采用大batch_size相近的效果。
    # Model parameters
    ...
    #本质上也是为了防止过拟合
    #L2正则和Weight Decay并不等价。这两者常常被大家混为一谈。首先两者的目的都是想是使得模型权重接近于0。L2正则是在损失函数的基础上增加L2 norm， 即为[公式] 。而权重衰减则是在梯度更新时直接增加一项， [公式] 。在标准SGD的情况下，通过对衰减系数做变换，可以将L2正则和Weight Decay看做一样。但是在Adam这种自适应学习率算法中两者并不等价。
    ...
    #详细解释见https://www.jianshu.com/p/1867381de345
    # Dataset parameters
    parser.add_argument('--data_path', default='./dataset', type=str,
                        help='dataset path')

    parser.add_argument('--output_dir', default='./output_dir_final_pre',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_dir_final_pre',
